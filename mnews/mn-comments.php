<?php
  ...
  elseif (isset($_GET['action']) && $_GET['action'] == 'ajaxcall' && isset($_GET['file'])) {
    ...
    elseif (isset($_POST['action']) && $_POST['action'] == 'bulk') {
        ...
        foreach ($comments as $comments_line) {
          ...
          if (isset($_GET['q']) && (strlen($_GET['q']) > 2) && (stripos($var['comment_text'], $_GET['q']) === false)) continue;
            ...
            if (isset($_GET['p']) && !empty($_GET['p']) && $post['id'] != $_GET['p']) continue;
              ...
              $status .= ($var['status'] == 2) ? ' <p class="process-links hide c"><a href="./mn-comments.php?a=m&amp;s=1&amp;f=' . $post['id'] . '&amp;c=' . $var['comment_id'] . '&amp;t=' . $_SESSION['mn_token'] . '&amp;msg=' . $lang['comm_status1'] . '" class="ajaxcall approve">' . $lang['comm_process_approve'] . '</a> &middot; <a href="./mn-comments.php?a=m&amp;s=5&amp;&amp;f=' . $post['id'] . '&amp;c=' . $var['comment_id'] . '&amp;t=' . $_SESSION['mn_token'] . '&amp;msg=' . $lang['comm_status5'] . '" class="ajaxcall reject">' . $lang['comm_process_reject'] . '</a></p>' : '';
              $checked = ((isset($_GET['approve']) && $var['status'] == 2) || isset($_GET['check'])) ? ' checked="checked"' : '';
              
              
              if ($var['status'] == 5) $comment_links = '<a href="./mn-comments.php?a=m&amp;s=1&amp;f=' . $post['id'] . '&amp;c=' . $var['comment_id'] . '&amp;t=' . $_SESSION['mn_token'] . '&amp;msg=' . $lang['comm_status1'] . '" class="ajaxcall approve">' . $lang['comm_process_approve'] . '</a> &middot; ';
              else $comment_links = '';
      ...
      else overall_header($lang['comm_comments'], $lang['comm_comments'] . (isset($_GET['trash']) ? ': ' . $lang['uni_trash'] : ''), 'main');
...
<?php
  ...
  if (isset($admin_tmpl['comments_list']) && $admin_tmpl['comments_list'] && MN_LOGGED) {
    ...
    if (!empty($comments_result) || !empty($_GET['d']) || !empty($_GET['p']) || !empty($_GET['a']) || !empty($_GET['i']) || !empty($_GET['s']) || !empty($_GET['q'])) {
      echo (empty($_GET['d']) && empty($_GET['p']) && empty($_GET['a']) && empty($_GET['i']) && empty($_GET['s']) && empty($_GET['q'])) ? ' &middot; <span class="simurl" id="filter-viewer"> <img src="./stuff/img/icons/view-settings.png" alt="" width="16" height="16" /> ' . $lang['comm_filter_settings'] . '</span>' : ' &middot; <a href="./mn-comments.php" class="custom"><img src="./stuff/img/icons/view-settings-cancel.png" alt="" width="16" height="16" /> ' . $lang['comm_filter_cancel'] . '</a>';
    }
    echo '</div>';
