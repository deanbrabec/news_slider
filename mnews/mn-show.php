<?php
  ...
  if (is_dir(MN_ROOT . 'data/posts/') && file_exists(MN_ROOT . 'data/databases/config.php')) {
   ...
   @include_once MN_ROOT . 'data/databases/config.php';
    ...
    $mn_categories = load_basic_data('categories', '', true);
    $mn_tags = load_basic_data('tags', '', true);



    # check settings, choose the best option
    ...
    if (isset($_GET['mn_cat']) && !is_numeric($_GET['mn_cat'])) {
    	foreach ($mn_categories as $cat_id => $cat) {
    		if ($cat['friendly_url'] == $_GET['mn_cat']) $get_cat = $cat_id;
    		else continue;
    	}
    	if (!isset($get_cat)) $get_cat = '';
    }
    else {
    	$get_cat = @$_GET['mn_cat'];
    }

    $mn_cat = (isset($mn_cat) && !empty($mn_cat)) ? $mn_cat : $get_cat;
    ...
    if (isset($_GET['mn_tag'])) {
    	if (!is_numeric($_GET['mn_tag'])) {
	    	foreach ($mn_tags as $tag_id => $tag) {
	    		if ($tag['friendly_url'] == $_GET['mn_tag']) $mn_tag = $tag_id;
	    		else continue;
	    	}
	    }
	    else {
      ...
      if (!empty($pages)) {
        ...
        foreach ($pages_order as $page_id => $page_order) {
          ...
          $mn_menu .= str_ireplace(
          	...
          	array('<a href="' . generate_page_url($mn_url, array('id' => $page_id, 'friendly_url' => $pages_slugs[$page_id])) . '"' . $active_page . '>' . $pages[$page_id] . '</a>', $page_id, $pages[$page_id], $pages_slugs[$page_id]),
          	$menu_tmpl
          );
    ...
    elseif (isset($mn_mode) && ($mn_mode == 'advanced_search' || $mn_mode == 'advanced-search')) {
      ...
      foreach ($mn_categories as $cat_id => $c) {echo '<option value="' . $cat_id . '">' . encoding($c['name']) . '</option>';}
      echo '</select> ';
	...
	elseif (isset($mn_mode) && ($mn_mode == 'tagcloud' || $mn_mode == 'tags')) {
		...
		if (is_array($mn_tags) && !empty($mn_tags)) {
			...
			foreach ($mn_tags as $tag_id => $t) {
				...
				$tag_cloud .= '<a href="' . generate_tag_url($mn_url, $mn_tags[$tag_id]) . '" class="mn-tag mn-tag-' . $tag_class . '" title="' . $lang['posts_posts'] . ': ' . $posts_count[$tag_id] . '">' . $t['name'] . '</a>' . $tag_divider;
    ...
    elseif (file_exists(MN_ROOT . $file['posts'])) {
      ...
      foreach ($p_file as $p_line) {
        ...
        elseif (isset($mn_author) && $post['status'] > '2') continue;
          ...
          if (isset($mn_tag) && !empty($mn_tag)) {
            $post['tags_array'] = (!empty($post['tags'])) ? explode(',', trim($post['tags'])) : array();
            if (!in_array($mn_tag, $post['tags_array'])) continue;
          }
      ...
      if (!empty($posts)) {
        ...
        if ($mn_pagination) {
          ...
          if (isset($_GET['mn_cat'])) {
          	if (isset($conf['url_cat_base']) && $conf['url_cat_base'] != '') {
          		$mn_pagination_url = $conf['url_base'] . $conf['url_cat_base'] . '/' . $_GET['mn_cat'] . '/';
          	}
          	else {
          		$mn_pagination_url = $mn_pagination_url . 'mn_cat=' . $_GET['mn_cat'] . '&amp;';
          	}
          }
          if (isset($_GET['mn_tag'])) {
          ...
          if (isset($_GET['mn_cat']) && isset($conf['url_cat_base']) && $conf['url_cat_base'] != '') $pages_tmpl_content = str_replace('mn_p=', '', $pages_tmpl_content);
          if (isset($_GET['mn_tag']) && isset($conf['url_tag_base']) && $conf['url_tag_base'] != '') $pages_tmpl_content = str_replace('mn_p=', '', $pages_tmpl_content);
        ...
        if (isset($mn_mode) && $mn_mode == 'rss') {
          ...
          elseif (isset($_GET['mn_cat']) && !empty($mn_categories[$mn_cat]['name']) && @$conf['web_section_titles'] !== false) echo encoding('<p class="mn-info">' . str_replace('%c%', '"<strong>' . $mn_categories[$mn_cat]['name'] . '</strong>"', $lang['web_msg_cat_posts']) . '</p>');
          elseif (isset($mn_tag) && !empty($mn_tags[$mn_tag]) && @$conf['web_section_titles'] !== false) echo encoding('<p class="mn-info">' . str_replace('%t%', '"<strong>' . $mn_tags[$mn_tag]['name'] . '</strong>"', $lang['web_msg_tag_posts']) . '</p>');
