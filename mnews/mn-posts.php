<?php
  ...
  elseif (isset($_POST['action']) && $_POST['action'] == 'edit' && isset($_POST['id'])) {
    ...
    if (!empty($_POST['title']) && !empty($_POST['short_story'])) {
      ...
      $p_content = SAFETY_LINE . "\n" . DELIMITER . $_POST['id'] . DELIMITER . $timestamp . DELIMITER . check_text($_POST['title']) . DELIMITER . $post_friendly_url . DELIMITER . $author . DELIMITER . $_POST['cat'] . DELIMITER . $status . DELIMITER . $_POST['comments'] . DELIMITER . $var['views'] . DELIMITER . $post_tags . DELIMITER . $post_image . DELIMITER . '' . DELIMITER . '' . DELIMITER . $xfields_serialized . DELIMITER . "\n" . check_text($short_story) . DELIMITER . "\n" . check_text($full_story);
      ...
      foreach ($p_file as $p_line) {
        ...
        $posts_result .= '<tr>
          <td class="cell-post">
            ...
            &nbsp;<span class="links hide"><a href="./mn-posts.php?action=edit&amp;id=' . $var['id'] . '">' . $lang['uni_edit'] . '</a> &middot; <a class="fancy" href="./mn-posts.php?action=delete&amp;id=' . $var['id'] . '">' . $lang['uni_delete'] . '</a> &middot; <a href="./mn-posts.php?action=show-post&amp;id=' . $var['id'] . '" class="fancy">' . $lang['uni_show'] . '</a>' . $show_web . $approve_button . '</span>
          </td>
          <td>' . $author . '</td>
          <td>' . $category_name . '</td>
          <td>' . $status . '</td>
  ...
  <form action="./mn-posts.php" method="post" enctype="multipart/form-data" id="posts-add-edit" class="p-form">
    <fieldset>
      <table id="table-posts-add-edit">
        <tr>
          <td class="labels vat"><label for="title"><img src="./stuff/img/icons/title.png" alt="-" width="16" height="16" /> <?php echo $lang['posts_title'];?><span class="required">*</span></label></td>
          <td class="inputs"><input type="text" name="title" id="title" class="text slug-source" tabindex="1" value="<?php echo $var['title'];?>" />
          <?php
          	if (isset($conf['url_post']) && ($conf['url_post'] == 2 || $conf['url_post'] == 4 || strpos($conf['url_post'], '%postname%') !== false)) {
          	
          		if ($conf['url_post'] == 2) {
          			$post_url = $conf['url_base'] . '%postname%/';
          		}
          		elseif ($conf['url_post'] == 4) {
          			$post_url = $conf['url_base'] . date('Y') . '/' . date('d') . '/%postname%/';
          		}
          		else {
          			$rule_search = array('%post_id%', '%year%', '%month%', '%day%', '%hour%', '%minute%');
          			$rule_replace = array('ID', date('Y'), date('d'), date('m'), date('H'), date('i'));
          			$post_url = str_ireplace($rule_search, $rule_replace, $conf['url_post']);
          		}
          		
          	
          		echo '<div class="friendly-url">http://' . $_SERVER['SERVER_NAME'] . str_replace('%postname%', '<input type="text" name="slug" id="slug" value="' . $var['friendly_url'] . '" />', $post_url) . '</div>';
          	}
          ?>
          </td>
        </tr>
	...
	?>
      ...
      <div class="toggles">
        <p class="fr r">
          <?php if (!empty($xfields_rows)) echo '<span class="simurl toggle" rel="xfields-in"><img src="./stuff/img/icons/textfield.png" alt="-" width="16" height="16" /> ' . $lang['xfields_xfields'] . '</span> &middot; ';?>
          <span class="simurl" id="p-tags-viewer"><img src="./stuff/img/icons/tags.png" alt="-" width="16" height="16" /> <?php echo $lang['tags_tags'];?></span> &middot;
...
<?php
  ...
  if (isset($admin_tmpl['form_posts_list']) && $admin_tmpl['form_posts_list']) {
    ...
    if (user_auth('2', true)) {
      ...
      if (file_exists(MN_ROOT . $file['tags'])) echo ' &middot; <a href="./mn-tags.php" class="custom"><img src="./stuff/img/icons/tags.png" alt="" width="16" height="16" /> ' . $lang['tags_tags'] . '</a>';
    }
    if (empty($_GET['c']) && empty($_GET['t']) && empty($_GET['a']) && empty($_GET['d']) && empty($_GET['s']) && empty($_GET['q']) && empty($posts_result)) echo '';
    elseif (empty($_GET['c']) && empty($_GET['t']) && empty($_GET['a']) && empty($_GET['d']) && empty($_GET['s']) && empty($_GET['q'])) echo ' &middot; <span class="simurl" id="filter-viewer"> <img src="./stuff/img/icons/view-settings.png" alt="" width="16" height="16" /> ' . $lang['posts_filter_settings'] . '</span>';
    else echo ' &middot; <a href="./mn-posts.php" class="custom"><img src="./stuff/img/icons/view-settings-cancel.png" alt="" width="16" height="16" /> ' . $lang['posts_filter_cancel'] . '</a>';
    echo '</div>';
