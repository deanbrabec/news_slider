...
// ----- Load basic data for selected section to array ---------------------------------------------
//
function load_basic_data($required_file, $id = '', $complex = false) {
  global $file;
  if (file_exists(MN_ROOT . $file[$required_file])) {
    $output = array();
    $selected_file = file(MN_ROOT . $file[$required_file]);
    ...
    foreach ($selected_file as $single_line) {
      ...
      elseif (empty($single_line)) continue;
      	...
      	if ($complex) {
      		$output[$file_data[0]] = array('id' => trim($file_data[0]), 'name' => trim($file_data[1]), 'friendly_url' => trim($file_data[2]));
      	}
      	else {
      		$output[$file_data[0]] = trim($file_data[1]);
      	}
      }
    }
...
//
function assign_post_vars($title, $author, $cat, $short_story, $full_story, $date, $date_day, $date_month, $date_year, $date_hour, $date_min, $status, $comments, $action, $id, $counter, $xfields = array(), $friendly_url = '') {
  $result = array(
    'title' => $title, 'author' => $author, 'cat' => $cat, 'short_story' => $short_story, 'full_story' => $full_story, 'date' => $date, 'date_day' => $date_day, 'date_month' => $date_month, 'date_year' => $date_year, 'date_hour' => $date_hour, 'date_min' => $date_min, 'status' => $status, 'comments' => $comments, 'action' => $action, 'id' => $id, 'counter' => $counter, 'xfields' => $xfields, 'friendly_url' => $friendly_url
  );
  return $result;
}
...
//
// ----- Get info from $_SERVER['HTTP_USER_AGENT'] string -----------------------------------------------------
//
...
// ----- Generate post url --------------------------------------------------------------------
//
function generate_post_url($url, $arr) {

	global $conf;

	$url_base = (!isset($conf['url_base'])) ? '/' : $conf['url_base'];

	if (!isset($conf['url_post']) || $conf['url_post'] == 1) {
		return $url . 'mn_post=' . $arr['id'];
	}

	elseif (isset($conf['url_post']) && $conf['url_post'] == 2) {
		return $url_base . $arr['friendly_url'] . '/';
	}
	
	elseif (isset($conf['url_post']) && $conf['url_post'] == 3) {
		return $url_base . 'post/' . $arr['id'];
	}
	
	elseif (isset($conf['url_post']) && $conf['url_post'] == 4) {
		return $url_base . $arr['date_year'] . '/' . $arr['date_month'] . '/' . $arr['friendly_url'] . '/';
	}
	
	else {
	
		$url_search = array('%postname%', '%post_id%', '%year%', '%month%', '%day%', '%hour%', '%minute%');
		$url_replace = array($arr['friendly_url'], $arr['id'], $arr['date_year'], $arr['date_month'], $arr['date_day'], $arr['date_hour'], $arr['date_minute']);
	
		return str_ireplace($url_search, $url_replace, $conf['url_post']);
	}
...
// ----- Generate page url --------------------------------------------------------------------
//
function generate_page_url($url, $arr) {
	...
	if (!isset($conf['url_page']) || $conf['url_page'] == 1) {
		return $url . 'mn_page=' . $arr['id'];
	}

	elseif (isset($conf['url_page']) && $conf['url_page'] == 2) {
		return $url_base . 'page/' . $arr['friendly_url'] . '/';
	}
	
	elseif (isset($conf['url_page']) && $conf['url_page'] == 3) {
		return $url_base . 'page/' . $arr['id'];
	}
	
	else {
	
		$url_search = array('%pagename%', '%page_id%');
		$url_replace = array($arr['friendly_url'], $arr['id']);
	
		return str_ireplace($url_search, $url_replace, $conf['url_page']);
	}
...
//
function generate_category_url($url, $arr) {
	...
	if (!isset($conf['url_cat_base']) || $conf['url_cat_base'] == '') {
		return $url . 'mn_cat=' . $arr['id'];
	}
	
	else {
		return $url_base . $conf['url_cat_base'] . '/' . $arr['friendly_url'] . '/';
	}
...
// ----- Generate tag url --------------------------------------------------------------------
//
function generate_tag_url($url, $arr) {
	...
	if (!isset($conf['url_tag_base']) || $conf['url_tag_base'] == '') {
		return $url . 'mn_tag=' . $arr['id'];
	}
	
	else {
		return $url_base . $conf['url_tag_base'] . '/' . $arr['friendly_url'] . '/';
	}
...
function comment_tmpl($template, $url, $num) {
  ...
  if (!empty($comment['author_id'])) {
  	$author = '<a href="' . $url . 'mn_user=' . $comment['author_id'] . '">' . $comment['author_name'] . '</a>';
  	$profile = '<a href="' . $url . 'mn_user=' . $comment['author_id'] . '">$1</a>';
...
function login_screen($title, $info_text = '', $info = '') {
  ...
  $login_screen_replace = array(
    $action,
    $checkbox,
    '&copy; <a href="http://mnewscms.com/">MNews</a> 2008&ndash;' . date('Y') . ' | mineo <a href="http://mineo.sk/">webdesign</a>',
    $delimiter,
    $info_message,
    $inputs,
    $lng,
