<?php
  ...
  if (isset($_GET['action']) && $_GET['action'] == 'theme') {
    ...
    if (file_exists($theme_config)) {
      ...
      foreach ($theme as $key => $value) {
        ...
        elseif (substr($key, -4, 4) == '_img') {
          ...
          $var_help = (isset($theme_help) && is_array($theme_help) && array_key_exists($key, $theme_help) && $theme_help[$key] != '') ? ' <span class="help tooltip" title="' . $theme_help[$key] . '">?</span>' : '';
    ...
    if (isset($theme_variants)) {
    
    	$variants = '';
    	foreach ($theme_variants as $variant_name => $variant) {
    		$variants .= '<span class="simurl" onclick="';
    		
    		foreach ($variant as $k => $v) {
    			$variants .= '$(\'#' . $k . '\').attr(\'value\', \'' . $v . '\'); $(\'#preview-' . $k . '\').css(\'background-color\', \'' . $v . '\'); ';
    		}
    		$variants .= '">' . $variant_name . '</span><br />';
    	}
  ...
  elseif (isset($_POST['action']) && $_POST['action'] == 'theme') {
	...
	elseif (isset($_GET['action']) && $_GET['action'] == 'htaccess') {


		$base = str_ireplace(array('+', '.', '?', '(', ')'), array('\+', '\.', '\?', '\(', '\)'), $conf['url_base']);
		$htaccess_content = '';



		// POST rewrite rules
		if (isset($conf['url_post']) && $conf['url_post'] != 1) {

			$htaccess_content .= "\n# Post rewrite rule\n";

			if ($conf['url_post'] == 2) {
				$htaccess_content .= 'RewriteRule  ^' . $base . '([a-z0-9\-\_]+)/$      /index.php?mn_post=$1  [L,QSA]' . "\n";
			}
			elseif ($conf['url_post'] == 3) {
				$htaccess_content .= 'RewriteRule  ^' . $base . 'post/([0-9]+)$      /index.php?mn_post=$1  [L,QSA]' . "\n";
			}
			elseif ($conf['url_post'] == 4) {
				$htaccess_content .= 'RewriteRule  ^' . $base . '([0-9]{4}+)/([0-9]{1,2}+)/([a-z0-9\-\_]+)/$      /index.php?mn_post=$3  [L,QSA]' . "\n";
			}
			else {
			
				$parse_array = explode('%', $conf['url_post']);
				$var_pos = array_search('postname', $parse_array);
				if (empty($var_pos)) $var_pos = array_search('post_id', $parse_array);
				$var_pos = ($var_pos+1)/2;
				
				$rule_search = array('+', '.', '?', '(', ')', '%postname%', '%post_id%', '%year%', '%month%', '%day%', '%hour%', '%minute%');
				$rule_replace = array('\+', '\.', '\?', '\(', '\)', '([a-z0-9\-\_]+)', '([0-9]+)', '([0-9]{4}+)', '([0-9]{1,2}+)', '([0-9]{1,2}+)', '([0-9]{1,2}+)', '([0-9]{1,2}+)');
				
				$htaccess_content .= 'RewriteRule  ^' . str_ireplace($rule_search, $rule_replace, $conf['url_post']) . '$      /index.php?mn_post=$' . $var_pos . '  [L,QSA]' . "\n";
		...
		// PAGE rewrite rules
		if (isset($conf['url_page']) && $conf['url_page'] != 1) {
		
			$htaccess_content .= "\n# Page rewrite rule\n";
		
			if ($conf['url_page'] == 2) {
				$htaccess_content .= 'RewriteRule  ^' . $base . 'page/([a-z0-9\-\_]+)/$      /index.php?mn_page=$1  [L,QSA]' . "\n";
			}
			elseif ($conf['url_page'] == 3) {
				$htaccess_content .= 'RewriteRule  ^' . $base . 'page/([0-9]+)$      /index.php?mn_page=$1  [L,QSA]' . "\n";
			}
			else {
				
				$rule_search = array('+', '.', '?', '(', ')', '%pagename%', '%page_id%');
				$rule_replace = array('\+', '\.', '\?', '\(', '\)', '([a-z0-9\-\_]+)', '([0-9]+)');
				
				$htaccess_content .= 'RewriteRule  ^' . str_ireplace($rule_search, $rule_replace, $conf['url_page']) . '$      /index.php?mn_page=$1  [L,QSA]' . "\n";
		...
		// Categories & Tags rewrite rules
		if (isset($conf['url_cat_base']) && $conf['url_cat_base'] != '') {
			$htaccess_content .= "\n# Categories rules\n";
			$htaccess_content .= 'RewriteRule  ^' . $base . $conf['url_cat_base'] . '/([a-z0-9\-\_]+)/$      /index.php?mn_cat=$1  [L,QSA]' . "\n";
			$htaccess_content .= 'RewriteRule  ^' . $base . $conf['url_cat_base'] . '/([a-z0-9\-\_]+)/([0-9]+)$      /index.php?mn_cat=$1&mn_p=$2  [L,QSA] # pagination' . "\n";
		}
		if (isset($conf['url_tag_base']) && $conf['url_tag_base'] != '') {
			$htaccess_content .= "\n# Tags rules\n";
			$htaccess_content .= 'RewriteRule  ^' . $base . $conf['url_tag_base'] . '/([a-z0-9\-\_]+)/$      /index.php?mn_tag=$1  [L,QSA]' . "\n";
			$htaccess_content .= 'RewriteRule  ^' . $base . $conf['url_tag_base'] . '/([a-z0-9\-\_]+)/([0-9]+)$      /index.php?mn_tag=$1&mn_p=$2  [L,QSA] # pagination' . "\n";
		}



		$htaccess_content = (!empty($htaccess_content)) ? "RewriteEngine on\n" . $htaccess_content : '';

		echo '<h1 class="htaccess">' . $lang['config_url_htaccess_heading'] . '</h1>';
		echo '<p class="l htaccess-text">' . $lang['config_url_htaccess_text'] . '</p>';
		echo '<h2 class="htaccess">.htaccess</h2>';
		echo '<pre class="example-htaccess"><code>' . $htaccess_content . '</code></pre>';
		echo '<p class="l htaccess-text">' . $lang['config_url_htaccess_text2'] . '</p>';
  ...
  elseif (isset($_POST['action']) && $_POST['action'] == 'save') {
    ...
    $url_base = (isset($_POST['url_base']) && !empty($_POST['url_base'])) ? check_text($_POST['url_base']) : '/';
    $url_post = ($_POST['url_post'] == 'x') ? "'".$_POST['url_post_custom']."'" : $_POST['url_post'];
    $url_page = ($_POST['url_page'] == 'x') ? "'".$_POST['url_page_custom']."'" : $_POST['url_page'];
  ...
  if (substr($url_path, -1) == '/') $url_path = substr($url_path, 0, -1);
	...
	$url_domain = 'http://' . $_SERVER['SERVER_NAME'];	
  $url_base = (isset($conf['url_base']) && !empty($conf['url_base'])) ? $conf['url_base'] : '/';
...
?>
  ...
  <script type="text/javascript">
    $(document).ready( function() {
    	$('#t').mnTabs({
    		defautContent: <?php echo (isset($_GET['t']) && is_numeric($_GET['t']) && ($_GET['t'] > 0) && ($_GET['t'] <= 6)) ? $_GET['t'] : '1';?>
    	});
    	$('#auto-url').click(function() {$('#admin_url').attr('value', '<?php echo $url_path;?>').focus();});
    });
  </script>
    ...
    <div id="t">
    	<div class="t-nav">
          ...
          <li id="t-nav-3"><?php echo $lang['config_tab_3'];?></li>
        	...
        	<li id="t-nav-5"><?php echo $lang['config_tab_5'];?></li>
        	<li id="t-nav-6" class="last"><?php echo $lang['config_tab_6'];?></li>
      	</ul>
    	</div>
      ...
      <div id="t-6" class="t-content"><div class="config-area">
      
      
      	<p><?php echo $lang['config_url_text'];?></p>
      	
      	<?php
      	
      		if ((isset($conf['url_post']) && $conf['url_post'] != 1) || (isset($conf['url_page']) && $conf['url_page'] != 1) || (isset($conf['url_cat_base']) && $conf['url_cat_base'] != '') || (isset($conf['url_tag_base']) && $conf['url_tag_base'] != '')) {
      		
      			echo '<p class="c"><a href="./mn-config.php?action=htaccess" class="fancy htaccess-link">' . $lang['config_url_htaccess_link'] . '</a></p>';
      	...
      	<ul class="config-edit">
      		<li><label><strong><?php echo $lang['config_url_base'];?></strong> <span class="domain"><?php echo $url_domain;?></span><input type="text" name="url_base" id="url_base" class="txt-input" value="<?php echo (isset($conf['url_base'])) ? $conf['url_base'] : '/';?>" /> <img src="./stuff/img/icons/help.png" alt="?" class="tooltip" title="<?php echo $lang['config_url_base_help'];?>" /></label></li>
      	</ul>
      
      	<h3 class="config-edit"><?php echo $lang['config_url_posts'];?></h3>

      	<ul class="config-edit">
      		<li><label><strong><input type="radio" class="radio" id="url_post1" name="url_post" value="1"<?php if ($conf['url_post'] == 1 || !isset($conf['url_post'])) echo ' checked="checked"';?>> <?php echo $lang['config_url_label_default'];?></strong> <span class="domain"><?php echo $url_domain . '</span><span class="url"><span class="url-base">' . $url_base;?></span>?mn_post=123</span></label></li>
      		<li><label><strong><input type="radio" class="radio" id="url_post2" name="url_post" value="2"<?php if ($conf['url_post'] == 2) echo ' checked="checked"';?>> <?php echo $lang['config_url_label_post_name'];?></strong> <span class="domain"><?php echo $url_domain . '</span><span class="url"><span class="url-base">' . $url_base;?></span>post-friendly-url/</span></label></li>
      		<li><label><strong><input type="radio" class="radio" id="url_post3" name="url_post" value="3"<?php if ($conf['url_post'] == 3) echo ' checked="checked"';?>> <?php echo $lang['config_url_label_numeric'];?></strong> <span class="domain"><?php echo $url_domain . '</span><span class="url"><span class="url-base">' . $url_base;?></span>post/123</span></label></li>
      		<li><label><strong><input type="radio" class="radio" id="url_post3" name="url_post" value="4"<?php if ($conf['url_post'] == 4) echo ' checked="checked"';?>> <?php echo $lang['config_url_label_date_name'];?></strong> <span class="domain"><?php echo $url_domain . '</span><span class="url"><span class="url-base">' . $url_base . '</span>' . date('Y') . '/' . date('d') . '/';?>post-friendly-url/</span></label></li>
      		<li><label><strong><input type="radio" class="radio" id="url_postx" name="url_post" value="x"<?php if (isset($conf['url_post']) && $conf['url_post'] != 1 && $conf['url_post'] != 2 && $conf['url_post'] != 3 && $conf['url_post'] != 4) echo ' checked="checked"';?>> <?php echo $lang['config_url_label_custom'];?></strong> <span class="domain"><?php echo $url_domain;?></span><input type="text" name="url_post_custom" id="url_post_custom" class="txt-input" value="<?php echo (isset($conf['url_post']) && !is_numeric($conf['url_post'])) ? $conf['url_post'] : '/%year%/%month%/%postname%/';?>"/></label> <img src="./stuff/img/icons/help.png" alt="?" class="tooltip" title="<?php echo $lang['config_url_variables'];?>:<br />%postname%, %post_id%, %year%, %month%, %&shy;day%, %hour%, %minute%" /></li>
      	</ul>
      	
      	
      	
      	<h3 class="config-edit"><?php echo $lang['config_url_pages'];?></h3>
      	
      	<ul class="config-edit">
      		<li><label><strong><input type="radio" class="radio" id="url_page1" name="url_page" value="1"<?php if ($conf['url_page'] == 1 || !isset($conf['url_page'])) echo ' checked="checked"';?>> <?php echo $lang['config_url_label_default'];?></strong> <span class="domain"><?php echo $url_domain . '</span><span class="url"><span class="url-base">' . $url_base;?></span>?mn_page=123</span></label></li>
      		<li><label><strong><input type="radio" class="radio" id="url_page2" name="url_page" value="2"<?php if ($conf['url_page'] == 2) echo ' checked="checked"';?>> <?php echo $lang['config_url_label_page_name'];?></strong> <span class="domain"><?php echo $url_domain . '</span><span class="url"><span class="url-base">' . $url_base;?></span>page/page-friendly-url/</span></label></li>
      		<li><label><strong><input type="radio" class="radio" id="url_page3" name="url_page" value="3"<?php if ($conf['url_page'] == 3) echo ' checked="checked"';?>> <?php echo $lang['config_url_label_numeric'];?></strong> <span class="domain"><?php echo $url_domain . '</span><span class="url"><span class="url-base">' . $url_base;?></span>page/123</span></label></li>
      		<li><label><strong><input type="radio" class="radio" id="url_pagex" name="url_page" value="x"<?php if (isset($conf['url_page']) && $conf['url_page'] != 1 && $conf['url_page'] != 2 && $conf['url_page'] != 3 && $conf['url_page'] != 4) echo ' checked="checked"';?>> <?php echo $lang['config_url_label_custom'];?></strong> <span class="domain"><?php echo $url_domain;?></span><input type="text" name="url_page_custom" id="url_page_custom" class="txt-input" value="<?php echo (isset($conf['url_page']) && !is_numeric($conf['url_page'])) ? $conf['url_page'] : '/%pagename%.html';?>" /></label> <img src="./stuff/img/icons/help.png" alt="?" class="tooltip" title="<?php echo $lang['config_url_variables'];?>:<br />%pagename%, %page_id%" /></li>
      	</ul>
      	
      	
      	
      	<h3 class="config-edit"><?php echo $lang['config_url_cats_tags'];?></h3>
      		
      		<p class="config-edit"><?php echo str_ireplace(array('%example_base%', '%example_url%', '%default_url%'), array('<span class="example">' . $lang['config_url_example_base'] . '</span>', '<span class="example">' . $url_domain . '<span class="url-base">' . $url_base . '</span><b>' . $lang['config_url_example_base'] .'</b>/tag-friendly-url/</span>', '<span class="example">?mn_tag=123</span>'), $lang['config_url_cats_tags_help']);?></p>
      		
      		<ul class="config-edit">
      			<li><label><strong><?php echo $lang['config_url_cats'];?></strong> <input type="text" name="url_cat_base" id="url_cat_base" class="txt-input" value="<?php echo @$conf['url_cat_base'];?>" /></label></li>
      			<li><label><strong><?php echo $lang['config_url_tags'];?></strong> <input type="text" name="url_tag_base" id="url_tag_base" class="txt-input" value="<?php echo @$conf['url_tag_base'];?>" /></label></li>
      		</ul>
